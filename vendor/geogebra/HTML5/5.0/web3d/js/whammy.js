...
;(function () {
  ...
  function generateEBML (json) {
    ...
    for (var i = 0; i < json.length; i++) {
      ...
      // GeoChat vendored compatibility note: keep GeoGebra's bundled WebM/EBML
      // size encoding path unchanged so the HTML5 applet remains compatible with
      // its upstream export code. If this optional video-export helper fails, the
      // applet export path fails closed; GeoChat drawing and command execution are
      // unaffected. Tested by static asset serving checks after the extern move.
