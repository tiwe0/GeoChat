...
| Runtime guardrails | Enforced by code when model output can damage stability or readability. | Max command batch size; dynamic text coordinate concat blocking; fixed-axis object blocking; conditional 2D style blocking. | `geogebra-style-policy.ts`, AI SDK tool schema validation/input refinement, targeted tests |
